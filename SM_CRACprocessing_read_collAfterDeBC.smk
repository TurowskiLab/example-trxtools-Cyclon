...
rule all:
	input:
		expand("01_preprocessing/11b_{sample}_flexbar.fastq.gz",sample=SAMPLES),
		expand("01_preprocessing/11c_{sample}_collapsed.fasta.gz",sample=SAMPLES),
		expand("12_alignment/{sample}_read.bam",sample=SAMPLES),
		expand("12_alignment/{sample}_read.bam.bai",sample=SAMPLES),
		"03_FetaureCounts/featureCounts_read_collAfterDeBc_multimappers.list",
		"03_FetaureCounts/featureCounts_read_collAfterDeBc_uniq.list",
		expand("14_BigWig/{sample}_read_CPM_plus.bw",sample=SAMPLES),
		expand("14_BigWig/{sample}_read_CPM_minus.bw",sample=SAMPLES),
		expand("14_BigWig/{sample}_read_plus.bw",sample=SAMPLES),
		expand("14_BigWig/{sample}_read_minus.bw",sample=SAMPLES),
...
rule debarcoding:
	...
	params:
		bcLen = bcLen
	output:
		"01_preprocessing/11a_{sample}_deBC.fastq.gz"
	conda:
		"envs/processing.yml"
	shell:
...
rule flexbar_3end_trimming:
	...
	params:
		adSeq = adSeq,
		n = "01_preprocessing/11b_{sample}_flexbar"
	output:
		"01_preprocessing/11b_{sample}_flexbar.fastq.gz"
	conda:
		"envs/flexbar.yml" 
	shell:
...
rule collapsing:
	...
	params:
		maxLen = maxLen
	output:
		"01_preprocessing/11c_{sample}_collapsed.fasta.gz"
	conda:
		"envs/processing.yml"
	shell:
...
rule align:
	input:
		reads_all = "01_preprocessing/11c_{sample}_collapsed.fasta.gz",
	params:
		index_dir = STAR_INDEX,
		prefix_all = "12_alignment/{sample}_read_STAR_"
	output:
		bam_all = "12_alignment/{sample}_read_STAR_Aligned.out.bam"
	conda:
		"envs/processing.yml"
	shell:
...
rule sort:
	input:
		bam = "12_alignment/{sample}_read_STAR_Aligned.out.bam"
	output:
		bam = "12_alignment/{sample}_read.bam",
		bai = "12_alignment/{sample}_read.bam.bai"
	conda:
		"envs/processing.yml"
	shell:
		"""
...
rule featureCounts:
	input:
		bam = expand("12_alignment/{sample}_read.bam",sample=SAMPLES) #use list of files
	output:
		multi = "03_FetaureCounts/featureCounts_read_collAfterDeBc_multimappers.list",
		uniq = "03_FetaureCounts/featureCounts_read_collAfterDeBc_uniq.list"
	params:
		gtf=GTF
	conda:
		"envs/processing.yml"
...
rule BigWigs_CPM:
	input:
		...
		bai = "12_alignment/{sample}_read.bam.bai"
	output:
		bwP = "14_BigWig/{sample}_read_CPM_plus.bw",
		bwM = "14_BigWig/{sample}_read_CPM_minus.bw"
	conda:
		"envs/processing.yml"
	shell:
		"""
...
rule BigWigs_read:
	input:
		...
		bai = "12_alignment/{sample}_read.bam.bai"
	output:
		bwP = "14_BigWig/{sample}_read_plus.bw",
		bwM = "14_BigWig/{sample}_read_minus.bw"
	conda:
		"envs/processing.yml"
	shell:
		"""
